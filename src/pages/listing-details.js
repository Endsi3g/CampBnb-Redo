...
export function listingDetailsPage(params = {}) {
  ...
  waitForElement('#listing-container')
    .then(() => loadListing(listingId))
    .catch(err => {
      console.error('Failed to initialize listing page:', err);
      // Fallback UI if container exists but waitForElement fails (unlikely if container exists, but good for timeout)
      const container = document.getElementById('listing-container');
      if (container) {
        container.innerHTML = `
            <div class="min-h-screen flex flex-col items-center justify-center p-6 text-center">
                <span class="material-symbols-outlined text-gray-500 text-6xl mb-4">error_outline</span>
                <p class="text-gray-400 mb-6">Something went wrong loading this page.</p>
                <button id="reload-listing-btn" class="bg-primary text-background-dark px-6 py-3 rounded-xl font-bold">
                    Reload
                </button>
            </div>
        `;
        const reloadBtn = container.querySelector('#reload-listing-btn');
        if (reloadBtn) {
